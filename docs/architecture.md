...
4. The result is a tuple of `Segment` values for the consumer.
...
4. Stored explicit topics are aggregated with distance decay. `supports` returns evidence only when the requested topic reaches the threshold.
