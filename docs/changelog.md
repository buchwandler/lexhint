# Changelog

> **Generated by releaseledger.** Manual edits may be overwritten. Update releaseledger entries and rebuild this file instead.

## [v0.4.8] - 2026-09-09

### Added

- Added -e and -n shortcuts for selecting English and native Wiktionary source variants

### Changed

- Changed dataset removal to select a sole matching source and require an explicit source for ambiguity

### Fixed

- Fixed dataset resolution so source, capability, and version filters preserve source preference and capability ranking

### Documentation

- Documented source-variant aliases, automatic selection, five-dimensional artifact identity, and release smoke checks

### Quality

- Improved regression coverage for source preference under capability and version selectors

<!-- generated by releaseledger -->

## [v0.4.7] - 2026-09-09

### Changed

- Changed installed dataset inventory and validation ordering to be deterministic across source and capability variants

### Fixed

- Fixed human dataset management output to distinguish Wiktionary source variants and removed duplicate dataset versions

### Documentation

- Documented dataset identity, advanced CLI controls, pronunciation filtering, locale semantics, and environment variables

### Quality

- Improved regression coverage for source selection, dataset output, CLI help, aliases, ordering, and language defaults

<!-- generated by releaseledger -->

## [v0.4.6] - 2026-09-09

### Added

- Added coexisting native and English dataset source variants with CLI/API selection
- Added full locale tags, Portuguese regional profiles, broader language coverage, and IPA filtering

<!-- generated by releaseledger -->

## [v0.4.5] - 2026-09-08

### Added

- Added Greek, Indonesian, Kurdish, Malay, Polish, and Turkish base-language support

### Documentation

- Documented project status badges for package metadata and quality visibility

<!-- generated by releaseledger -->

## [v0.4.4] - 2026-09-03

### Added

- Added Japanese, Korean, Russian, Thai, Vietnamese, and Chinese language support

<!-- generated by releaseledger -->

## [v0.4.3] - 2026-09-02

### Added

- Added a lazy bulk iterator for deterministic pronunciation exports from dictionary artifacts

### Changed

- Improved focused pronunciation output with canonical IPA brackets and case-variant grouping
- Added locale-aware pronunciation preference with neutral fallback when no locale match exists

<!-- generated by releaseledger -->

## [v0.4.2] - 2026-08-31

### Added

- Added static catalog support for selecting and downloading compatible dataset artifacts
- Added grouped pronunciation lookup with locale, region, and part-of-speech filtering
- Added cached catalog discovery and dataset update commands for compatible releases

<!-- generated by releaseledger -->

## [v0.4.1] - 2026-08-31

### Added

- Added support for language-qualified dataset releases and language-aware artifact validation

<!-- generated by releaseledger -->

## [v0.4.0] - 2026-08-25

### Added

- Added deterministic sense identities, source provenance, and sense lookup by ID
- Added entry-level dictionary relations with incoming relation lookup

### Changed

- Added a search-free dictionary artifact profile and immutable schema 10 finalization checks
- **BREAKING:** Changed schema 10 into an exact artifact boundary with side-by-side schema families and full managed-variant rebuilds

<!-- generated by releaseledger -->

## [0.3.0] - 2026-08-24

### Changed

- Added explicit headword relations for redirects, alternative spellings, and form-of entries

<!-- generated by releaseledger -->

## [0.2.1] - 2026-08-23

### Changed

- Added the managed dictionary dataset variant for full dictionary inspection without indexed search

<!-- generated by releaseledger -->

## [0.2.0] - 2026-08-23

### Added

- Added schema-8 fuzzy suggestions, headword pattern matching, and indexed dictionary definition search

<!-- generated by releaseledger -->

## [0.1.3] - 2026-08-22

### Added

- Added deterministic normalized lexical prefix completion through Lexicon.complete() and the complete CLI command

### Changed

- Added automatic color styling for interactive human dictionary output with explicit opt-out controls

<!-- generated by releaseledger -->

## [0.1.2] - 2026-08-21

### Added

- Added deterministic local lexical prefix completion through Lexicon.complete() and lexhint complete

### Changed

- Changed WordEvidence to expose lexical case attestation and uppercase-only evidence

### Fixed

- Fixed semantic context anchoring so character-span numeric and symbolic targets do not consume adjacent cues

<!-- generated by releaseledger -->

## [0.1.1] - 2026-08-21

### Added

- Added GitHub release dataset discovery and installation for versioned Lexhint SQLite artifacts

### Changed

- Added schema-aware dataset variants and regional locale preferences for dictionary lookup and rendering

<!-- generated by releaseledger -->

## [0.1.0] - 2026-08-20

### Added

- Added CLI commands for lookup, segmentation, context evidence, artifact builds, dictionary inspection, and status
- Added streamed Wiktextract and Kaikki dictionary builds that produce self-describing SQLite artifacts
- Added stable semantic-domain evidence that explains context cues while excluding the target span
- Added a local read-only Lexicon for dictionary-derived membership and compact identifier segmentation
- Added zero runtime dependencies and support for Python 3.10 through 3.14

### Changed

- Changed FrequencyWords integration to optional counts and ranks that enrich dictionary-derived lexemes with provenance
- Changed rich dictionary entries to preserve senses, forms, pronunciations, examples, relations, and etymology

### Removed

- Removed standalone word-list vendoring in favor of dictionary-derived lexical membership
- Removed lazy runtime dictionary fetching and per-word artifact population

### Documentation

- Documented the local SQLite artifact architecture and Sphinx documentation setup

### Quality

- Improved CI and publication checks for tests, static analysis, documentation, distributions, and wheel smoke tests

<!-- generated by releaseledger -->

<!-- releaseledger:generated-file -->
